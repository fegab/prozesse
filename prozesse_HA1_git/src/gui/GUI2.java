...
public class GUI2 implements ActionListener, TreeSelectionListener {
	...
	private JButton btnRelation = new JButton("Relation!");
	...
	public GUI2(Collection<Menge> mengen) {
		...
		frame.add(btnRelation);
		frame.add(btnGenerateMenge);
		...
		if (e.getSource().equals(btnRelation)) {
			...
			if (leftPath != null && rightPath != null) {
				...
				if (leftElement instanceof Element
					...
					Relation relation = new Relation(new Sense("test"),
							leftElement, rightElement);

					DefaultMutableTreeNode nodeRelation0 = new DefaultMutableTreeNode(
							relation);
					DefaultMutableTreeNode nodeRelation1 = new DefaultMutableTreeNode(
							relation);
					...
					System.out
							.println("relation added: " + relation.toString());
				}
	...
	private void createTree(Collection<Menge> mengen) {
		...
		for (Menge menge : mengen) {
			...
			for (UUID uuid : menge.getElemente().keySet()) {
				...
				DefaultMutableTreeNode nodeElement = new DefaultMutableTreeNode(
						element);
